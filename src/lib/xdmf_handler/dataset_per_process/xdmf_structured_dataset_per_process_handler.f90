...
private
    ...
    !-----------------------------------------------------------------
    !< XDMF dataset per process handler for structured Grids
    !----------------------------------------------------------------- 
    contains
    private
...
contains
    ...
    end subroutine xdmf_str_dataset_per_process_SetTopology_I4P


    subroutine xdmf_str_dataset_per_process_SetTopology_I8P(this, Connectivities, Name)
    !-----------------------------------------------------------------
    ...
    !-----------------------------------------------------------------
        ...
        call UniformGridDescriptor%SetTopologyMetadata(Name            = Name, &
                                                       Precision       = 8,    &
                                                       ArrayDimensions = (/size(Connectivities, dim=1)/))
    end subroutine xdmf_str_dataset_per_process_SetTopology_I8P


    subroutine xdmf_str_dataset_per_process_FillSpatialGridTopology(this, TopologyNode, ID)
    !-----------------------------------------------------------------
    ...
    end subroutine xdmf_str_dataset_per_process_WriteTopology


    subroutine xdmf_str_dataset_per_process_WriteGeometry(this, GridID)
    !-----------------------------------------------------------------
    ...
    end subroutine xdmf_str_dataset_per_process_WriteGeometry


    subroutine xdmf_str_dataset_per_process_WriteGeometry_DXDYDZ(this, GridID)
    !-----------------------------------------------------------------
    ...
    !-----------------------------------------------------------------
        ...
        if(MPIEnvironment%is_root()) then
            ...
            call dataitem%open(xml_handler = this%GetSpatialFileXMLHandler(), &
                    Dimensions = (/int(SpaceDimension,I8P)/), &
                    NumberType = 'Float', &
                    Format     = 'HDF', &
    ...
    end subroutine xdmf_str_dataset_per_process_WriteGeometry_DXDYDZ


    subroutine xdmf_str_dataset_per_process_WriteGeometry_VXVYVZ(this, GridID)
    !-----------------------------------------------------------------
    !< Write a XDMF Geometry into a opened file for the dataset per process strategy
    !----------------------------------------------------------------- 
        class(xdmf_structured_dataset_per_process_handler_t), intent(INOUT) :: this !< XDMF dataset per process handler for structured Grids
    ...
    end subroutine xdmf_str_dataset_per_process_WriteGeometry_VXVYVZ


end module xdmf_structured_dataset_per_process_handler
